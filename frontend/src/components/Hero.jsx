...
function Hero() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      // Update mouse position for state
      ...
      if (cursorRef.current) {
        ...
        // Dynamic color change based on position
  ...
  // Show loading state until mounted - but don't put any hooks after this
  if (!mounted) {
    return (
  ...
  return (
    ...
    >
      {/* Canva-style Dynamic Cursor Lighting Effect - Desktop Only */}
      {!isMobile && (
        <>
          <motion.div
            ref={cursorRef}
      ...
      {/* Dynamic Background with Mouse-Responsive Gradients */}
      ...
      {/* PROPERLY CENTERED Content Layer */}
        ...
        <div className="text-center w-full max-w-6xl">
          ...
          >
            <motion.h1
              ...
              className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl font-black leading-tight"
            >
            ...
            <motion.div
              ...
              className="h-1 bg-gradient-to-r from-blue-500 to-purple-500 mt-4 mx-auto rounded-full"
            />
          </motion.div>
          ...
          <motion.p
            ...
            className="text-lg sm:text-xl md:text-2xl text-gray-300 mb-8 md:mb-12 max-w-2xl md:max-w-3xl mx-auto leading-relaxed font-light"
          >
            Where <span className="text-cyan-300 font-medium">innovation</span> meets{" "}
            <span className="text-purple-300 font-medium">design</span>. Creating digital experiences 
          ...
          >
            <motion.button
              ...
              className="px-8 py-4 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold text-lg shadow-2xl transition-all duration-300 group relative overflow-hidden"
            >
            ...
            <motion.button
              ...
              className="px-8 py-4 rounded-full border-2 border-purple-500/30 bg-purple-500/5 backdrop-blur-sm text-white font-bold text-lg hover:bg-purple-500/10 transition-all duration-300 group"
            >
          ...
          {/* Stats Section - Hidden on mobile, shown on tablet+ */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 1, delay: 1.5 }}
            className="hidden sm:grid grid-cols-3 gap-8 max-w-2xl mx-auto mt-12"
          >
            {[
      ...
      {!isMobile && (
        ...
        >
          <motion.div
            ...
            className="flex flex-col items-center text-gray-400 hover:text-cyan-300 cursor-pointer group"
          ...
          >
            ...
            <div className="w-6 h-10 border-2 border-gray-600 rounded-full flex justify-center group-hover:border-cyan-400 transition-colors">
              <motion.div
                animate={{ y: [0, 6, 0] }}
                transition={{ duration: 2, repeat: Infinity }}
                className="w-1 h-3 bg-gray-600 rounded-full mt-2 group-hover:bg-cyan-400 transition-colors"
              />
            </div>
          </motion.div>
        </motion.div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <motion.div
          ...
          className="absolute bottom-32 right-16 w-3 h-3 bg-purple-400 rounded-full"
        />
      </div>
    </section>
  );
